...
describe Coin2Coin::Message::Association do
  before do
  ...
  describe "validations" do
    ...
    describe "data_store_identifier_has_correct_permissions" do
      ...
      context "when data_store_identifier does not allow requests" do
        before do
          Coin2Coin::DataStore.instance.stub(:identifier_can_request?).and_return(false)
        end
      ...
      context "when not read only and data_store_identifier cannot insert" do
        let(:read_only) { false }
        before { Coin2Coin::DataStore.instance.stub(:identifier_can_insert?).and_return(false) }
      ...
      context "when not read only and data_store_identifier can insert" do
        let(:read_only) { false }
        before { Coin2Coin::DataStore.instance.stub(:identifier_can_insert?).and_return(true) }
  ...
  describe "build" do
    subject { Coin2Coin::Message::Association.build(coin_join, name, type, read_only) }
    ...
    it "has a data_store_identifier_from_build that can insert and request" do
      expect(Coin2Coin::DataStore.instance.identifier_can_insert?(subject.data_store_identifier_from_build)).to be_true
      expect(Coin2Coin::DataStore.instance.identifier_can_request?(subject.data_store_identifier_from_build)).to be_true
    end
    ...
    context "when read-only" do
      ...
      it "has a data_store_identifier that can only request" do
        expect(Coin2Coin::DataStore.instance.identifier_can_insert?(subject.data_store_identifier)).to be_false
        expect(Coin2Coin::DataStore.instance.identifier_can_request?(subject.data_store_identifier)).to be_true
      end
    end
    ...
    context "when not read-only" do
      ...
      it "has a data_store_identifier that can insert and request" do
        expect(Coin2Coin::DataStore.instance.identifier_can_insert?(subject.data_store_identifier)).to be_true
        expect(Coin2Coin::DataStore.instance.identifier_can_request?(subject.data_store_identifier)).to be_true
      end
  ...
  describe "from_data_store_identifier" do
    ...
    subject do
      Coin2Coin::Message::Association.from_data_store_identifier(data_store_identifier, coin_join, name, type, read_only)
    end
